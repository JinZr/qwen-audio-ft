...
class Metrics:
    ...
    def compute(self, predictions, references, normalize=True, sample_weight=None):
        ...
        if self.num_classes == 1:
            metric_dic["rmse"] = metrics.root_mean_squared_error(
                y_true=references, y_pred=predictions, squared=False
            )
        else:
            ...
            if self.num_classes == 2:
                ...
                metric_dic["precision"] = metrics.precision_score(
                    references, predictions
                )
                metric_dic["recall"] = metrics.recall_score(references, predictions)
            else:
                metric_dic["f1"] = metrics.f1_score(
                    references, predictions, average="macro"
                )
                metric_dic["precision"] = metrics.precision_score(
                    references, predictions, average="macro"
                )
                metric_dic["recall"] = metrics.recall_score(
                    references, predictions, average="macro"
                )
